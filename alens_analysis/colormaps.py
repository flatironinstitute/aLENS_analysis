...
def create_cmaps(cmap_data):
    ...
    for name, spec in cmap_data.items():
        cmap_d[name] = (
            ...
            if "red" in spec
            else mpl.colors.ListedColormap(spec["listed"], name)
            if "listed" in spec
            else mpl.colors.LinearSegmentedColormap.from_list(name, spec, lutsize)
        )
    ...
    for k, v in cmap_d.items():
        mpl.colormaps.register(name=k, cmap=v)
